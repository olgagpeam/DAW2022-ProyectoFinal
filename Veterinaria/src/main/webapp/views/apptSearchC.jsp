<%-- 
    Document   : apptSearchC
    Created on : Jun 13, 2022, 12:29:30 PM
    Author     : olgag
--%>
<%-- 
    ...
    <body>
        <div id="container">
            ...
            <div id="form">
                ...
                <%
                    String[] res = (String[]) session.getAttribute("apptCRes");
                    ...
                    if ((user != null && user.size() > 0) && (owner != null && owner.size() > 0)
                            && (pet != null && pet.size() > 0)) {
                %>
                <form method="post" action="/Veterinaria/ApptConsultUp">
                    ...
                    <select name="r_consult" class="sel">
                        <option class="opt" value="" <%if ("Domicilio".equals(res[8])) {%> selected <%}%> > Domicilio </option>
                        <option class="opt" value="" <%if ("Veterinaria".equals(res[8])) {%> selected <%}%> > Veterinaria </option>
                    </select>
                    
                    <input type="text" name="adrrRef" <% if (res[9] == null || "".equals(res[9])) {%> placeholder="Referencias domicilio">
                    <%} else {%> value="<%=res[9]%>">
                    <%}%>
                    
                    <input type="text" name="diagnosis" <% if (res[10] == null || "".equals(res[10])) {%> placeholder="Diagnostico">
                    <%} else {%> value="<%=res[10]%>">
                    <%}%>
                    <input type="text" name="procedures" <% if (res[11] == null || "".equals(res[11])) {%> placeholder="Procedures">
                    <%} else {%> value="<%=res[11]%>">
                    <%}%>
                    
                    <input type="text" name="med" <% if (res[12] == null || "".equals(res[12])) {%> placeholder="Medicamentos">
                    <%} else {%> value="<%=res[12]%>">
                    <%}%>
